...
_valid_required() {
    local required_cmds=("xz" "pgrep" "curl" "tar" 'unzip' 'jq')
    local missing=()
...
_prompt_install_dir() {
    _okcat '📍' "默认安装目录: $CLASH_BASE_DIR"
    echo -n "$(_okcat '🤔' '是否安装到默认目录? [Y/n]: ')"
    read -r choice
    case "$choice" in
        [nN]|[nN][oO])
            echo -n "$(_okcat '📂' '请输入自定义安装路径 (如 /data/xds/mihomo): ')"
            read -r input_path
            [ -z "$input_path" ] && _error_quit "未输入有效路径"
            [[ "$input_path" == ~* ]] && input_path="${HOME}${input_path:1}"
            [[ "$input_path" != /* ]] && input_path="/$input_path"
            CLASH_BASE_DIR="$input_path"
            sed -i "s|^CLASH_BASE_DIR=.*|CLASH_BASE_DIR=\"$CLASH_BASE_DIR\"|" ".env"
            _refresh_dirs
            _okcat '👌' "安装目录已更新: $CLASH_BASE_DIR"
            ;;
    esac
}
